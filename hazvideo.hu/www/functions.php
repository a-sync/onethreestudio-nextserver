...
function mail_kuldo($cimzett, $targy, $uzenet, $valaszcim = 'noreplay@hazvideo.hu') {
  $headers ='From: Hazvideo.hu <ertesito@hazvideo.hu>'."\r\n"
    .'Reply-To: '.$valaszcim."\r\n"
    .'Return-Path: '.$valaszcim."\r\n"
    .'X-Mailer: PHP/'.phpversion()."\r\n";

    $uzenet .= "\r\n\r\n".'www.hazvideo.hu - `Hirdesse lakását biztonságosan és jutalékmentesen.`'."\r\n\n";
...
function adatok_szurese($tomb, &$hibak, $escape = false, $feladas = false) {
  ...
  // kategória
  if(!is_numeric($tomb['kategoria']) || $data['kategoria'][$tomb['kategoria']] == '') {
    //$hibak['kategoria'] = 'Érvénytelen kategória típus.';
    $tomb['kategoria'] = '';
  }
  //else $tomb['kategoria'] = substr($tomb['kategoria'], 0, 10);
  
  // jelleg
  if(!is_numeric($tomb['jelleg']) || $data['jelleg'][$tomb['jelleg']] == '') {
    //$hibak['jelleg'] = 'Érvénytelen jelleg típus.';
    $tomb['jelleg'] = '';
  }
  //else $tomb['jelleg'] = substr($tomb['jelleg'], 0, 10);
  
  // szobák
  $tomb['szobak'] = str_replace(array(',', ' '), array('.', ''), $tomb['szobak']);
  if(!is_numeric($tomb['szobak'])) {
    //$hibak['szobak'] = 'A szobák számának numerikus értéknek kell lennie.';
    $tomb['szobak'] = '';
  }
  else $tomb['szobak'] = number_format(substr($tomb['szobak'], 0, 10), 1, '.', '');
  $tmp_array = explode('.', $tomb['szobak']);
  if($tmp_array[1] == 0) $tomb['szobak'] = $tmp_array[0];
  //var_dump($tomb['szobak']);exit;//debug
  //if($tomb['szobak'] == 0) $tomb['szobak'] = '';
  //$tomb['szobak'] = '123';
  
  // alapterület
  if(!is_numeric($tomb['alapterulet'])) {
    $hibak['alapterulet'] = 'Az alapterületnek numerikus értéknek kell lennie, négyzetméterben értve.';
    $tomb['alapterulet'] = '';
  }
  else $tomb['alapterulet'] = substr($tomb['alapterulet'], 0, 10);
  ...
  // szintek
  $tomb['szintek'] = str_replace(array(',', ' '), array('.', ''), $tomb['szintek']);
  if(!is_numeric($tomb['szintek'])) {
    //$hibak['szintek'] = 'A szintek számának numerikus értéknek kell lennie.';
    $tomb['szintek'] = '';
  }
  else $tomb['szintek'] = number_format(substr($tomb['szintek'], 0, 10), 1, '.', '');
  $tmp_array = explode('.', $tomb['szintek']);
  if($tmp_array[1] == 0) $tomb['szintek'] = $tmp_array[0];
  
  // garázs
  if(!is_numeric($tomb['garazs']) || $data['garazs'][$tomb['garazs']] == '') {
    //$hibak['garazs'] = 'Érvénytelen garázs típus.';
    $tomb['garazs'] = '';
  }
  //else $tomb['garazs'] = substr($tomb['garazs'], 0, 10);
...
// ?regio=0&telepules=0&varosresz=0&kategoria=2&jelleg=1
// &szobak_tol=1&szobak_ig=10&alapterulet_tol=2&alapterulet_ig=20
// &ar_tol=3&ar_ig=30&szintek_tol=4&szintek_ig=40&garazs=1
  global $data;
  ...
  // kategória
  if(is_numeric($tomb['kategoria']) && $data['kategoria'][$tomb['kategoria']] != '') {
    $tomb['kategoria'] = mysql_real_escape_string($tomb['kategoria']);
    $where .= " AND `kategoria` = '{$tomb['kategoria']}'";
    $link .= '&amp;kategoria='.$tomb['kategoria'];
  }
  
  // jelleg
  if(is_numeric($tomb['jelleg']) && $data['jelleg'][$tomb['jelleg']] != '') {
    $tomb['jelleg'] = mysql_real_escape_string($tomb['jelleg']);
    $where .= " AND `jelleg` = '{$tomb['jelleg']}'";
    $link .= '&amp;jelleg='.$tomb['jelleg'];
  }
  
  // szobáktól
  if(is_numeric($tomb['szobak_tol'])) {
    $tomb['szobak_tol'] = mysql_real_escape_string($tomb['szobak_tol']);
    $where .= " AND (`szobak` >= '{$tomb['szobak_tol']}' OR `szobak` = '0.0')";
    $link .= '&amp;szobak_tol='.$tomb['szobak_tol'];
  }
  
  // szobákig
  if(is_numeric($tomb['szobak_ig'])) {
    $tomb['szobak_ig'] = mysql_real_escape_string($tomb['szobak_ig']);
    $where .= " AND (`szobak` <= '{$tomb['szobak_ig']}' OR `szobak` = '0.0')";
    $link .= '&amp;szobak_ig='.$tomb['szobak_ig'];
  }
  
  // alapterülettől
  if(is_numeric($tomb['alapterulet_tol'])) {
    $tomb['alapterulet_tol'] = mysql_real_escape_string($tomb['alapterulet_tol']);
    $where .= " AND `alapterulet` >= '{$tomb['alapterulet_tol']}'";
    $link .= '&amp;alapterulet_tol='.$tomb['alapterulet_tol'];
  }
  
  // alapterületig
  if(is_numeric($tomb['alapterulet_ig'])) {
    $tomb['alapterulet_ig'] = mysql_real_escape_string($tomb['alapterulet_ig']);
    $where .= " AND `alapterulet` <= '{$tomb['alapterulet_ig']}'";
    $link .= '&amp;alapterulet_ig='.$tomb['alapterulet_ig'];
  }
  ...
  // szintektől
  if(is_numeric($tomb['szintek_tol'])) {
    $tomb['szintek_tol'] = mysql_real_escape_string($tomb['szintek_tol']);
    $where .= " AND (`szintek` >= '{$tomb['szintek_tol']}' OR `szintek` = '0.0')";
    $link .= '&amp;szintek_tol='.$tomb['szintek_tol'];
  }
  
  // szintekig
  if(is_numeric($tomb['szintek_ig'])) {
    $tomb['szintek_ig'] = mysql_real_escape_string($tomb['szintek_ig']);
    $where .= " AND (`szintek` <= '{$tomb['szintek_ig']}' OR `szintek` = '0.0')";
    $link .= '&amp;szintek_ig='.$tomb['szintek_ig'];
  }
  
  // garázs
  if(is_numeric($tomb['garazs']) && $data['garazs'][$tomb['garazs']] != '') {
    $tomb['garazs'] = mysql_real_escape_string($tomb['garazs']);
    $where .= " AND `garazs` = '{$tomb['garazs']}'";
    $link .= '&amp;garazs='.$tomb['garazs'];
  }
          ...
          ?>
            ...
            <div class="reg_row">
                ...
                <div class="reg_col_right">
                  ...
                  <div class="reg_col_right_row">
                    <select name="kategoria" id="reg_kategoria">
                      <!--<option value="">Kategóriák...</option>-->
                      <?php
                        foreach($data['kategoria'] as $key => $val) {
                          if($hirdetes['kategoria'] == $key) $key .= '" selected="selected';
                          echo '<option value="'.$key.'">'.$val.'</option>';
                        }
                      ?>
                    </select>
                  ...
                  <div class="reg_col_right_row">
                    <select name="jelleg" id="reg_jelleg">
                      <!--<option value="">Jelleg...</option>-->
                      <?php
                        foreach($data['jelleg'] as $key => $val) {
                          if($hirdetes['jelleg'] == $key) $key .= '" selected="selected';
                          echo '<option value="'.$key.'">'.$val.'</option>';
                        }
                      ?>
                    </select>
                  ...
                  <div class="reg_col_right_row"><input name="szobak" id="reg_szobak" type="text" maxlength="10" value="<?php echo $hirdetes['szobak']; ?>" onkeyup="numeric_input(this, 1);" /></div>
                  
                  <div class="reg_col_right_row"><input name="alapterulet" id="reg_alapterulet" type="text" maxlength="10" value="<?php echo $hirdetes['alapterulet']; ?>" rel="tip:Négyzetméterben értendő (kötelező kitölteni)" onkeyup="numeric_input(this, 0);"/></div>
                  
                  <div class="reg_col_right_row"><input name="ar" id="reg_ar" type="text" maxlength="10" value="<?php echo $hirdetes['ar']; ?>" rel="tip:Bérelhető ingatlan esetén havidíjként értendő (kötelező kitölteni)" onkeyup="numeric_input(this, 0);"/></div>
                  
                  <div class="reg_col_right_row"><input name="szintek" id="reg_szintek" type="text" maxlength="10" value="<?php echo $hirdetes['szintek']; ?>" onkeyup="numeric_input(this, 1);" /></div>
                  
                  <div class="reg_col_right_row">
                    <select name="garazs" id="reg_garazs">
                      <!--<option value="">Garázs...</option>-->
                      <?php
                        foreach($data['garazs'] as $key => $val) {
                          if($hirdetes['garazs'] == $key) $key .= '" selected="selected';
                          echo '<option value="'.$key.'">'.$val.'</option>';
                        }
                      ?>
                    </select>
...
//todo: ha admin akkkor az admin adatokat is vegye fel
  ...
  $datum = time();
  $datum2 = '946684800';
  $hirdetes_cime = $data['jelleg'][$hirdetes['jelleg']].' '.$data['kategoria'][$hirdetes['kategoria']];
  if($hirdetes['szobak'] != '') $hirdetes_cime .= ', '.$hirdetes['szobak'].' szoba';
  
  $q = mysql_query("INSERT INTO `hirdetesek` (`hiid`, `hirdetes_cime`, `regio`, `telepules`, `varosresz`, `kategoria`, `jelleg`, `szobak`, `alapterulet`, `ar`, `szintek`, `garazs`, `megjegyzes`, `cim`, `statusz`, `datum`, `ervenyes_datum`, `email`, `jelszo`, `nev`, `telefon`, `kiemelt_datum`, `uzletkoto`) VALUES (NULL, '{$hirdetes_cime}', '{$hirdetes['regio']}', '{$hirdetes['telepules']}', '{$hirdetes['varosresz']}', '{$hirdetes['kategoria']}', '{$hirdetes['jelleg']}', '{$hirdetes['szobak']}', '{$hirdetes['alapterulet']}', '{$hirdetes['ar']}', '{$hirdetes['szintek']}', '{$hirdetes['garazs']}', '{$hirdetes['megjegyzes']}', '{$hirdetes['cim']}', '0', '{$datum}', '{$datum2}', '{$hirdetes['email']}', '{$hirdetes['jelszo']}', '{$hirdetes['nev']}', '{$hirdetes['telefon']}', '{$datum2}', '')");
...
function hirdetes_modositasa($hirdetes) {
  ...
  $query = "UPDATE `hirdetesek` SET `regio` = '{$hirdetes['regio']}', `telepules` = '{$hirdetes['telepules']}', `varosresz` = '{$hirdetes['varosresz']}', `kategoria` = '{$hirdetes['kategoria']}', `jelleg` = '{$hirdetes['jelleg']}', `szobak` = '{$hirdetes['szobak']}', `alapterulet` = '{$hirdetes['alapterulet']}', `ar` = '{$hirdetes['ar']}', `szintek` = '{$hirdetes['szintek']}', `garazs` = '{$hirdetes['garazs']}', `megjegyzes` = '{$hirdetes['megjegyzes']}', `cim` = '{$hirdetes['cim']}', `jelszo` = '{$hirdetes['jelszo']}', `nev` = '{$hirdetes['nev']}', `telefon` = '{$hirdetes['telefon']}'$admin_settings WHERE `hiid` = '{$hirdetes['hiid']}' LIMIT 1";
...
function kategoriak_kiirasa() { // valamifele cach-elést be lehetne iktatni
  global $data;
  $time = time();
  $n = 0;
  
  echo '<ul>';
  foreach($data['kategoria'] as $id => $nev) {
    $where = " WHERE `statusz` > '1' AND `ervenyes_datum` > '$time' AND `kategoria` = '$id'";
    $hirdetesek_szama = end(mysql_fetch_assoc(mysql_query("SELECT COUNT(`hiid`) as 'osszes' FROM `hirdetesek`$where")));
    $n += $hirdetesek_szama;
    
    echo '<li><a href="hirdetesek.php?kategoria='.$id.'">'.$nev.' ('.$hirdetesek_szama.' db)</a></li>';
  }
  echo '</ul>';
...
function makeThumbnail($originalImage, $thumbnailImage, $toWidth = 120, $toHeight = 80, $fixWidth = false, $fixHeight = false, $resamplingQ = 100, $debug = false) {
  ...
  //plain copy to protect from errors if GDlib is not present (and debug level is at minimum)
  if(!function_exists('imagecreatetruecolor') && $debug === false) {
    //protect from ppl trying to use this for hacking
    if($o_ext != 'png' && $o_ext != 'gif' && $o_ext != 'jpeg' && $o_ext != 'jpg') {
      return false; // unsupported file format
    }
    else {
      return copy($originalImage, $thumbnailImage);
    }
  }
  ...
  $error['$imageResized'] = 'imagecreatetruecolor('.$new_width.', '.$new_height.')';
...
//die('d2');
        if(!is_file($mappa.$kicsi) || $rethumb) {
          $d = makeThumbnail($mappa.$elem, $mappa.$kicsi, 120, 180, false, false, 100, false);
...
//die('d3');
    ...
    if((!is_file($mappa.'kicsi.jpg') || $rekicsi !== false) && $kepek[0][1] != '') {
      ...
      if(false === copy($mappa.rawurldecode($kepek[$rekicsi][0]), $mappa.'kicsi.jpg')) {
        $d = makeThumbnail($mappa.rawurldecode($kepek[$rekicsi][0]), $mappa.'kicsi.jpg', 120, 80, 120, 80, 100, false);
        //dbg($d, __FUNCTION__, __FILE__, __LINE__);
      }
    }
