...
if($_GET['kilepes'] != '' && is_numeric($_SESSION['admin']['hiid'])) {
  uj_esemeny('Kijelentkezés. (ID: '.$_SESSION['admin']['hiid'].')', $_SESSION['admin']['hiid'], 0, 0);
...
if($hirdeteseim_szama == 1 && $ne_listazzon) {
  ...
  if($_POST['rpass'] == $hirdetes['jelszo']) {
    ...
    uj_esemeny('Bejelentkezés. (ID: '.$_SESSION['admin']['hiid'].')', $_SESSION['admin']['hiid'], 0, 0);
    
    header("Location: hirdetes-modositasa.php?hirdetes={$hirdetes['hiid']}");
    exit;
  }
  elseif($_POST['rpass'] != '') {
    uj_esemeny('Sikertelen bejelentkezési kísérlet. (IP: '.$_SERVER['REMOTE_ADDR'].'). (ID: '.$hirdetes['hiid'].')', $hirdetes['hiid'], 1, 0);
  }
}
