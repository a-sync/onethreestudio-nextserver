...
<form action="" method="post">
  ...
  <br/>
  Prioritás: <input type="text" name="priority" maxlength="4" size="4" value="<?php echo htmlspecialchars($cat['priority']); ?>"/>
...
<?php
  ...
  $cat_result = mysql_query("SELECT * FROM `galeria` ORDER BY `priority` DESC, `name` ASC");
?>
...
<?php
  while($cat = mysql_fetch_assoc($cat_result)) {
    ...
    echo '<a href="?cat='.$cat['cat_id'].'">'.htmlspecialchars($cat['name']).'</a> ('.$cat['priority'].')<br/>';
