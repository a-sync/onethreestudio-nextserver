...
Latest File Update: 2009-Jan-29
*/
...
$furl=$furlCl="{$forum}_{$topic}_{$page}.html#msg{$anchor}";
$topicsLink="<a href=\"{$main_url}/{$forum}_0.html\">{$l_returntotopics}</a><br />";
} else {
$furl="{$indexphp}action=vthread&amp;forum=$forum&amp;topic=$topic&amp;page={$page}#msg{$anchor}";
$furlCl="{$indexphp}action=vthread&forum=$forum&topic=$topic&page={$page}#msg{$anchor}";
$topicsLink="<a href=\"{$main_url}/{$indexphp}action=vtopic&amp;forum={$forum}\">{$l_returntotopics}</a><br />";
}
$correctErr="<a href=\"{$main_url}/{$furl}\">$l_back</a>";
}

if(isset($editMsgReloc)) {
header("Refresh: 0; url={$main_url}/{$furlCl}"); exit;
}
else {
echo load_header(); echo ParseTpl(makeUp('main_warning')); return;
}
