...
Latest File Update: 2009-Jan-23
*/
...
if(isset($mod_rewrite) and $mod_rewrite) $urlp="{$lastT[1]}_{$lastT[0]}_0.html"; else $urlp="{$indexphp}action=vthread&amp;forum={$lastT[1]}&amp;topic={$lastT[0]}&amp;page=0";

$whatValue.="<li><a href=\"{$main_url}/{$urlp}\"{$nof}>{$lastT[2]}</a></li>";
}
...
if(isset($mod_rewrite) and $mod_rewrite) $urlp="{$k}_0.html"; else $urlp="{$indexphp}action=vtopic&amp;forum={$k}&amp;page=0";

if($stats_barWidth>$stats_barWidthLim) $key="<a href=\"{$main_url}/{$urlp}\" {$nof}>{$forumNames[$k]}</a>";
else{
$key2="<a href=\"{$main_url}/{$urlp}\" {$nof}>{$forumNames[$k]}</a>";
$key="<a href=\"{$main_url}/{$urlp}\" {$nof}>...</a>";
}
