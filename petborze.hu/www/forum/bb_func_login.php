...
Latest File Update: 2007-Sep-06
*/
...
$urlp=preg_replace("@".str_replace('?', '\?', $indexphp)."action=vthread&forum=([0-9]+)&topic=([0-9]+)&page=([-0-9]+)&mdrw=on(#newreply)?@", "\\1_\\2_\\3.html\\4", $urlp);
$urlp=preg_replace("@".str_replace('?', '\?', $indexphp)."action=vtopic&forum=([0-9]+)&page=([-0-9]+)&mdrw=on(#newtopic)?@", "\\1_\\2.html\\3", $urlp);
$urlp=preg_replace("@".str_replace('?', '\?', $indexphp)."mdrw=on@", $startIndex, $urlp);
}
