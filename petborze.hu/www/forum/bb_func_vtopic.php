...
Latest File Update: 2008-Oct-30
*/
...
if(isset($mod_rewrite) and $mod_rewrite and $sortBy==0) $urlp="{$forum}_{$realPage}.html";
else $urlp="{$indexphp}action=vtopic&forum=$forum&page={$realPage}";

header("HTTP/1.1 301 Moved Permanently");

if(isset($metaLocation)) {
$meta_relocate="{$main_url}/{$urlp}"; echo ParseTpl(makeUp($metaLocation)); exit;
}
else{
...
$pageNav=pageNav($page,$numRows,$urlp,$viewmaxtopic,FALSE,($user_sort==0 and isset($mod_rewrite) and $mod_rewrite));
if($pageNav!='') $mbpn='mb';
...
$pageNavCell=pageNav(0,$numReplies+1,$urlp,$viewmaxreplys,TRUE,(isset($mod_rewrite) and $mod_rewrite));
