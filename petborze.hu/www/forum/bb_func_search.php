...
Latest File Update: 2009-Jan-13
*/
...
if($pt<=$viewmaxreplys) $page=0;
elseif((integer)($pt/$viewmaxreplys)==($pt/$viewmaxreplys)) $page=$pt/$viewmaxreplys-1;
else $page=(integer)($pt/$viewmaxreplys);

if(isset($mod_rewrite) and $mod_rewrite) $furl="{$forum}_{$topic}_{$page}.html#msg{$anchor}";
else $furl="{$indexphp}action=vthread&forum={$forum}&topic={$topic}&page={$page}#msg{$anchor}";
...
$posterName=htmlspecialchars(trim($_GET['posterName']),ENT_QUOTES);
...
if(isset($mod_rewrite) and $mod_rewrite) $furl="{$row[1]}_{$row[0]}_0.html";
else $furl="{$indexphp}action=vthread&amp;forum={$row[1]}&amp;topic={$row[0]}&amp;page=0";

$searchResults.=<<<out
<table class="tbTransparent" style="width:100%"><tr><td class="tbTransparentCell">{$num}. <span class="txtSm"><strong>{$l_posted}</strong>: {$datetime} - <strong>{$forum_name}</strong></span> / <a href="{$main_url}/{$furl}">{$txt}</a>
</td></tr></table><br />
out;
$num++;
}
